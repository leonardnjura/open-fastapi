...
import google.generativeai as genaiOlSkul
...
class LipaNaMpesaStkPush(BaseModel):
    businessShortCode: int
    tillNo: int
    accessToken: str
    base64Encodedpassword: str
    timestamp: str
    transactionType: str
    customerPhone: int
    amt: int
...
def chat_about_image_with_gemini(request: Request, item: ImageChatGemini):
    ...
    genaiOlSkul.configure(api_key=my_google_gemini_api_key)
    ...
    try:
        ...
        model = genaiOlSkul.GenerativeModel(model_id)
        ...
        if '429 You exceeded your current quota' in verbose:
            msg = f"Exceeded quota | {my_google_gemini_api_key}"

        raise HTTPException(status_code=500, detail={
                            "msg": msg, "verbose": verbose})

                            
@app.post("/api/topnix/stk")
def topnix_init_stk_push(request: Request, item: LipaNaMpesaStkPush):
    access_token = item.accessToken
    daraja_base_url = item.darajaBaseUrl
    daraja_endpoint = f'{daraja_base_url}/mpesa/stkpush/v1/processrequest'
    ...
    payload = {
        ...
        "Password": item.base64Encodedpassword,
        "Timestamp": item.timestamp,
        "TransactionType": item.transactionType,
        "Amount": item.amt,
        "PartyA": item.customerPhone,
        "PartyB": item.tillNo,
    ...
    response = requests.request("POST", daraja_endpoint, headers = headers, json = payload)
    ...
    return {
        ...
        "verbose": {
            "mode": "python & daraja stk push --take IV",
            "access_token": access_token,
            "daraja_endpoint": daraja_endpoint,
            "daraja_req_payload": payload,
        }
    }


@app.post("/api/topnix/callback")
async def topnix_callback_url(request: Request):
    body = await request.body()
    try:
        print(f'callback_body: \n{body}\n')
        return {
            "verbose": "log stk body to db -> body.Body.stkCallback",
        }
    except:
        traceback.print_exc()
        msg = "Server says urgh----"
...
@app.post("/api/topnix/result")
async def topnix_callback_url(request: Request):
    body = await request.body()
    try:
        print(f'result callback_body: \n{body}\n')
        return {
            "verbose": "log transactionstatus body to db -> body.?.?",
        }
    except:
        traceback.print_exc()
        msg = "Server says urgh----"
...
@app.post("/api/topnix/queue")
async def topnix_callback_url(request: Request):
    body = await request.body()
    try:
        print(f'queue callback_body: \n{body}\n')
        return {
            "verbose": "log transactionstatus body to db -> body.?.?",
        }
    except:
...
@app.post("/api/ai/image-chat-pro")
def chat_about_image_with_gemini_pro(request: Request, item: ImageChatGemini):
    # gemini api key placed in here as endpoint is public
    # pass yours in header as my-google-gemini-api-key
    # TODO jwt / cookies auth
    # GOOGLE_GEMINI_API_KEY = os.getenv('GOOGLE_GEMINI_API_KEY')
    ...
    client = genai.Client(
        api_key=my_google_gemini_api_key,
    )
...
@app.post("/api/ai/image-gen-gpt")
async def image_gen_using_gpt(request: Request, item: ImageGenGpt):
    # TODO jwt / cookies auth
...
async def image_gen_using_gemini(request: Request, item: ImageGenGemini):
    # TODO jwt / cookies auth
    # GOOGLE_GEMINI_API_KEY = os.getenv('GOOGLE_GEMINI_API_KEY') || os.environ.get("GOOGLE_GEMINI_API_KEY")
    my_google_gemini_api_key = request.headers.get('my-google-gemini-api-key')
    default_model_id = "models/imagen-4.0-generate-preview-06-06"
    ...
    try:
        ...
        client = genai.Client(api_key=my_google_gemini_api_key)
        ...
        result = client.models.generate_images(
            ...
            config=dict(
                ...
                aspect_ratio="1:1",  # others 16:9, 9:16, 4:3, 3:4
            ),
        )
        ...
        for generated_image in result.generated_images:
            ...
            b64_json = base64.b64encode(image_bytes)
            result_list.append(b64_json)

        first_b64_json = result_list[0]

        return {
            "message": "all fields",
            "data": {
                "modelId": model_id,
                "b64Json": first_b64_json,
                "genType": "gemini",
                "prompt": command_prompt,
                "conversationId": "xxxxxx="
            },
        ...
        if 'API is only accessible to billed users' in verbose:
            msg = f"Enable billing | {my_google_gemini_api_key}"
...
def get_one_post(shortcode: str):
    # regex for ig shortcode from urls? https://regex101.com/r/ZUHk60/3
    try:
